...
**최신 후속 작업(2026-09-12 22시):** 기존 재점검 자료를 `f982499`로 커밋하고 [가격 원인 추적](docs/strategy-research/price-provenance-review-2026-09-12.md)과 [HTS 대조 준비](docs/strategy-research/hts-comparison-ready-2026-09-12.md)를 진행했다. 820만 행·종가 0인 1,201행은 유지되며 현재 배포 코드 5개가 로컬과 일치한다. 원천 응답·실행 manifest는 미확보라 수익률 실행은 계속 차단한다. HTS 3종목 180봉의 SQL/Python 독립 검산은 통과했으나 실제 HTS 결과는 없다. 다음 입력은 백필 원천/완료 로그 위치와 동일 기준일 HTS 결과다. 아래 종료·원격·환경 기록은 과거 세션 기록이다.
...
**2026-09-12 후속 실측:** [백필 재점검](docs/strategy-research/backfill-review-2026-09-12.md)을 먼저 읽는다. 16:55~16:57 KST에 8,202,637행·4,506종목, 2015-01-02~2026-09-10을 확인했다. `102950`의 종가 0인 1,201행 때문에 현재 테이블 그대로의 수익률 실행은 `BLOCKED`다. 다음 작업은 이 행의 원천·변환·거래 상태와 백필 manifest 및 가격 조정 정의를 확보하는 것이다. E0 진단과 HTS 대조 준비는 가능하다. 아래 추천과 기존 수치는 당시 기록이며 최신 관측은 후속 보고서를 따른다.
