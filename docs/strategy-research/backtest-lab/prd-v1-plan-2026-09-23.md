...
| --- | --- | --- | --- | --- |
| 종목 일봉 | 부분 | 2014~2023-12-28 구간에 수정가 4,574,759행·2,312종목, 원가 5,099,694행·2,787종목이 있고 준비된 cohort 는 1,076종목·2,243,249행이다. | 원가 OHLC≤0 135,717행, 수정가 부재 475건, 250봉 미달 306건, 미해결 행사 160건, 미설명 수정계수 1,061건이 남아 있다. | [postgresql-readiness README](postgresql-readiness-2026-09-21/README.md), [cohort.json](../../../../vectorbt-data/krx-v3-20260921-prepared-r2/cohort.json) |
...
| 종목 마스터 이력 | 부분 | 식별자 이력 907,098행, 보통주 유효기간 31,475행(KOSPI 6,861·KOSDAQ 24,614)을 보유한다. | 전체 member 해시 재계산과 가격쌍 대조를 실행하지 않았고 ETF·우선주·스팩 구분 필드도 미확인이다. `security_type`·상장폐지·일별 거래상태가 로컬 Parquet 에 없고 PostgreSQL 뷰 `backtest_universe_v2` 에만 있다. | [postgresql-readiness README](postgresql-readiness-2026-09-21/README.md), [B1 Universe 진단](prd-v1-data-2026-09-23/README.md) |
...
| 수량 정산 | 차단 | OHLCV20 탐색에서 176건이 제외되었다. | [`corporate_actions.py`](../../../research/krx_lab/corporate_actions.py)의 `CorporateActionBook.process` 가 분할·병합만 처리하고 무상증자·주식배당 신주 수량을 정산하지 않는다. | [`corporate_actions.py`](../../../research/krx_lab/corporate_actions.py) |
| 거래 캘린더 | 확보 | 공식 달력 3,652일 중 개장일 2,459일을 보유한다. | 없다. | [calendar.parquet](../../../../vectorbt-data/krx-v3-20260921-prepared-r2/calendar.parquet) |
| 비용표 | 부분 | 실행 규칙 15행(수수료·매도세·호가·가격제한·결제)을 보유한다. | 수수료는 사용자 가정(0.015%)이고 거래세의 효력일별 이력 검증을 실행하지 않았다. | [rules.parquet](../../../../vectorbt-data/krx-v3-20260921-prepared-r2/rules.parquet) |
| 거래정지·유동성 | 부분 | 거래상태 292행(정지 279·무봉 정지 1·부분거래 12)을 보유한다. | 가격제한 도달 여부와 부분체결 판정은 엔진 쪽 작업으로 남아 있다. | [postgresql-readiness README](postgresql-readiness-2026-09-21/README.md) |
...
| B1b | PostgreSQL `backtest_universe_v2` 에서 `security_type`·상장·폐지·거래상태를 읽기 전용으로 재추출해 로컬 스냅샷에 추가한다. | 코드, 홈서버 DB 접근 | B1 | 대기(DB 접근 확인 필요) |
...
| B2b | 비용표를 `rules.parquet` 형식의 효력일별 표로 반영하고 법령 원문으로 재확인한다. | 코드, 외부 원천 | B2 | 대기 |
| B3 | 무상증자·주식배당 수량 정산을 `corporate_actions.py` 에 추가하고 합성 fixture 로 검증한다. | 코드 | 없음 | 대기 |
| B4 | 원가 OHLC≤0 135,717행을 거래정지·무거래와 대조해 설명되지 않는 행만 남긴다. | 코드 | B1 | 대기 |
...
| B7 | 종목에 귀속되지 않는 이슈 1,523행을 원천(ted-startup)과 연결해 종목·일로 귀속한다. | 외부 원천, 코드 | B1 | 대기 |

B1b 가 Universe 재구성의 선행 조건이며, B3 은 독립적으로 바로 시작할 수 있다.
