...
def build_signals(prices: pd.DataFrame) -> pd.DataFrame:
    """Return input rows plus ATR, current 20-day average volume, and 12 entry booleans.
    ...
    state does not bridge it.  No calendar dates are inserted here.
    """
    ...
    pieces = [_build_one(group.copy()) for _, group in work.groupby("code", sort=False, group_keys=False)]
...
def _build_one(frame: pd.DataFrame) -> pd.DataFrame:
