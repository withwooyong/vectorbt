...
- 다음: B2b(비용표 효력일별 반영) 또는 B7(미귀속 이슈 1,523행). DB 접속은 드라이버 없이 `research/krx_lab/v3_snapshot.py` 의 `COMMAND` 방식으로 된다. B3b 는 실행 경로를 건드리므로 `BLOCKED` 해제와 함께 다룬다. 계획서 §4 의 사용자 결정 D1~D4 는 미정이다.
